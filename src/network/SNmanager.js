...
SNmanager.prototype.makeSN = function (dataset) {
    var startIdx = this.attributes.indexOf('start_time');
    for (var idx = 0; idx < dataset.length; idx++) {
        var one_data = dataset[idx];
        var startNode = new Node(this.attributes[startIdx], one_data[startIdx]);
        this.nodeDict[this.attributes[startIdx], one_data[startIdx]] = startNode;
        ...
        for (var j = 0; j < this.attributes.length; j++) {
            ...
            if (one_data[j] == '/' || one_data[j] == '') {
                continue;
            }
            var tmpNode = new Node(this.attributes[j], one_data[j]);
            if ([this.attributes[j], one_data[j]] in this.nodeDict){
                continue;
            }
            this.nodeDict[this.attributes[j], one_data[j]] = tmpNode;
