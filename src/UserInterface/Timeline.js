...
Timeline.prototype.init = function (gssDict) {
    ...
    for (var gss_value = 0; gss_value < Object.values(gssDict).length; gss_value++) {
        var tmp_gss = (Object.values(gssDict))[gss_value];
        //시작시간에 대한 for문
        for (var st_idx = 0; st_idx < tmp_gss.children_list.length; st_idx++) {
            var tmp_st_date = tmp_gss.children_list[st_idx];
            //start_time = new Date(시작시간)
            var start_time = new Date(tmp_st_date.value);
            //end_time = new Date(종료시간)
            var tmp_end_date = tmp_st_date.getChildByAttr('end_time');
            if (tmp_end_date == null) {
                alert('end_time is null!');
            }
            var end_time = new Date(tmp_end_date.value);
            //activity_value = 행동  (end time 값, activity값 받고)
            var tmp_activity = tmp_st_date.getChildByAttr('activity');
