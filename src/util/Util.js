...
var SclabUtil = {
    ...
    readInputFile:function(startDate, days, dataIndex){
        var totalDataset = []; // 네트워크 형성에 쓰일 최종 데이터셋
        var tmp_date = startDate.split('-');
        var seedDate = new Date(tmp_date[0], tmp_date[1] -1, tmp_date[2]); // 기준날짜에 대한 date 객체를 얻는다.
        for(var i=0; i<days; i++){ // SN생성에 사용할만큼 반복

            //참조할 데이터파일의 이름 생성
            var year = seedDate.getFullYear() + '';
            var month = SclabUtil.pad(seedDate.getMonth() + 1,2) + '';
            var day = SclabUtil.pad(seedDate.getDate(),2) + '';
            var fileName =  year + month + day + '_' + SclabUtil.pad(dataIndex,3); // 생성된 파일 이름

            //파일 이름에 해당하는 csv파일의 모든 데이터셋을 로드하여 totalDataset에 concatenate한다
            var oneDataset = (this.getdata(fileName)).responseText;
...
//            console.log(totalDataset.length); // 데이터셋 잘들어가는지 디버깅용으로 찍어줌
        ...
        //각 행마다 for문
        var startTimeIndex = totalDataset[0].indexOf('start_time');
        var endTimeIndex = totalDataset[0].indexOf('end_time');
